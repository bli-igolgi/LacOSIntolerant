...
void paging_init()
{
    ...
    // Map the pages for the video memory for the 3 terminals and the regular video page
    for(index = 0; index <= MAX_TERM_NUM; index++) {
        map_page((void *)(VIDEO_ADDR + (index)*FOUR_KB),
                 (void *)(VIDEO_ADDR + (index)*FOUR_KB), 0, 0, 1, 0);
    }
